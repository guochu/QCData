...
with open(filename, "r", encoding="utf-8") as f:
    for line in f:
        if line.strip() == "" or line.startswith("E(") or line.startswith("bonddim"):
            continue
        parts = line.split()
        if len(parts) < 6:
            continue
...
plt.plot(chi_inv, d1, "o-", label="CA-DMRG", color="tab:red")
plt.plot(chi_inv, d2, "s-", label="DMRG", color="tab:blue")
